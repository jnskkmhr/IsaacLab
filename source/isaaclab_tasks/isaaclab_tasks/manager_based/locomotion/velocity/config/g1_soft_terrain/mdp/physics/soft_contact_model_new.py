...
class RFT_EMF:
    ...
    def create_internal_tensors(self):
        ...
        self.contact_point_normal = torch.zeros((self.num_envs, self.num_bodies, self.num_contact_points, 3), device=self.device)
    ...
    def _eval_contacts(self)->None:
        ...
        self.contact_point_normal = (R @ n.unsqueeze(-1)).squeeze(-1)
        ...
        n = self.contact_point_normal.clone()
        ...
        nr = self.contact_point_normal - (self.contact_point_normal * z).sum(dim=-1, keepdim=True) * z.squeeze(-1)
    ...
    def _get_normal_force(
        ...
        vn = (self.contact_point_lin_vel * (-self.contact_point_normal)).sum(dim=-1) # (num_envs, num_bodies, num_contact_points)
        fn = self.force_ema.reshape(self.num_envs, self.num_bodies, self.num_contact_points) + lam * rho * vn**2
        force_normal = (fn.unsqueeze(-1) * (-self.contact_point_normal)).reshape(self.num_envs, -1, 3)
    ...
    def _get_tangential_force(self, v:torch.Tensor, fn:torch.Tensor)->torch.Tensor:
        ...
        """
        vn = (v * (-self.contact_point_normal).reshape(self.num_envs, -1, 3)).sum(dim=-1) # (num_envs, num_bodies*num_contact_points)
        vt = v - vn.unsqueeze(-1) * (-self.contact_point_normal).reshape(self.num_envs, -1, 3) # (num_envs, num_bodies*num_contact_points, 3)
