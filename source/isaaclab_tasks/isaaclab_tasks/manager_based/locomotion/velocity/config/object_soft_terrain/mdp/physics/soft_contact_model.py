...
#         # self.filter_tangential_velocity(vt, vt_unit_vec, dt=1/200)
#         # vt = self.vt_filtered
...
class RFT_2D:
    ...
    def _eval_contacts(self)->None:
        ...
        # print("intrusion angle: ", self.contact_point_intrusion_angle) 
...
class RFT_3D:
    ...
    def _get_resistive_force(
        ...
        force = (force_vec[:, :, 0:1]+kdt*vr**2) * self.r_dir.reshape(self.num_envs, -1, 3) * (-1) + \
                (force_vec[:, :, 1:2]+kdt*vt**2) * self.t_dir.reshape(self.num_envs, -1, 3) * (-1) + \
