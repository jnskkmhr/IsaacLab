...
class NewtonNerdManager(NewtonManager):
    ...
    @classmethod
    def _capture_relaxed_graph(cls, device: str, capture_target=None):
        if cls._nerd_active:
            return None
        return super()._capture_relaxed_graph(device, capture_target)
    ...
    def _build_solver(cls, model, solver_cfg) -> None:
        ...
        neural_model = cls._load_nerd_neural_model(cfg_dict, NewtonManager._solver)
    ...
    def _prepare_nerd_contacts(cls):
        ...
        with step_profile.section("contact_prepare"):
            if cls._nerd_contact_mode == "fixed_ground":
                ...
                if NewtonManager._collision_pipeline is None or NewtonManager._contacts is None:
                    raise RuntimeError("NeRD newton_native contact pipeline has not been initialized.")
                NewtonManager._collision_pipeline.collide(cls._state_0, NewtonManager._contacts)
                return NewtonManager._contacts
            else:
    ...
    def sync_neural_solver(cls, *, update_history: bool = True) -> None:
        ...
        if not cls._nerd_active or NewtonManager._solver is None:
            return
        if cls._control is None:
            raise RuntimeError("Cannot sync NeRD solver before Newton control is initialized.")

        contacts = cls._prepare_nerd_contacts()
        sync_from_newton = getattr(NewtonManager._solver, "sync_from_newton", None)
    ...
    def _reset_solver_internals(cls, world_mask: wp.array | None) -> None:
        """Defer history reset to NeuralEnvAdapter after FK and input synchronization.

        NeRD reset accepts environment indices, unlike Newton's physical solver
        reset API. The adapter resets only the requested environments after their
        new generalized coordinates have been propagated to body transforms.
        """
    ...
    def reset_neural_solver(cls, env_ids=None, *, reset_history: bool = True) -> None:
        ...
        reset_solver = getattr(NewtonManager._solver, "reset", None)
