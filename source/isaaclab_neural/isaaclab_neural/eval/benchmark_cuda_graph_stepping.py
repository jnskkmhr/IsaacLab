...
This targets pure Newton tasks (e.g. ``Isaac-Cartpole`` with ``newton_mjwarp``)
...
def register_task_module(task: str) -> None:
    ...
    if task == "Isaac-Cartpole":
        import isaaclab_tasks.core.cartpole  # noqa: F401
    elif "NeRD" in task or "Dataset-Gen" in task:
        import isaaclab_neural.envs  # noqa: F401
    else:
        raise ValueError(
            f"Task {task!r} is not registered by this benchmark. Use Isaac-Cartpole or an isaaclab_neural task."
        )
...
def _inject_preset(hydra_args: list[str], preset: str) -> list[str]:
    ...
    is PhysX (e.g. ``Isaac-Cartpole``) must be switched to a Newton preset to run.
    """
    if not preset:
        return hydra_args
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser.add_argument("--task", type=str, default="Isaac-Cartpole", help="Registered gym task id.")
    ...
    from isaaclab.app.sim_launcher import add_launcher_args
...
def _run_benchmark_worker(payload: dict[str, object]) -> dict[str, dict[str, float]]:
    ...
    def _worker(env_cfg, _agent_cfg=None) -> None:
        ...
        from isaaclab.app.sim_launcher import launch_simulation
